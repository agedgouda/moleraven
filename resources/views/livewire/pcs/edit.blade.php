            ...
            <div class="rounded-xl border border-zinc-200 dark:border-zinc-700" x-data="{ tab: 'connections' }">
                <div class="border-b border-zinc-200 px-4 pb-1 dark:border-zinc-700">
                    ...
                    <div x-show="tab === 'connections'" class="p-6">
                        ...
                        @php
                            $allConnections = $this->npcConnections->map(fn($c) => ['type' => 'npc', 'conn' => $c])
                                ->concat($this->orgConnections->map(fn($c) => ['type' => 'org', 'conn' => $c]))
                                ->sortBy(fn($item) => $item['type'] === 'npc' ? $item['conn']->npc->name : $item['conn']->organization->name);
                        @endphp
                            ...
                            <div class="space-y-2">
                                @foreach ($allConnections as $item)
                                    ...
                                    <div class="flex items-start justify-between rounded-lg border border-zinc-100 px-3 py-2 dark:border-zinc-700">
                                        <div>
                                            @if ($type === 'npc')
                                                <a href="{{ route('npcs.edit', $conn->npc) }}" class="font-medium text-zinc-800 hover:text-blue-600 dark:text-zinc-200" wire:navigate>{{ $conn->npc->name }}</a>
                                                <span class="ml-1 text-xs text-zinc-400">NPC</span>
                                            @else
